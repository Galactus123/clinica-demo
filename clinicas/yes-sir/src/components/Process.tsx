            ...
            {clinic.process.map((item, index) => (
              <div key={index} className="relative flex flex-col items-center text-center">
                {index < clinic.process.length - 1 && (
                  <div className="hidden md:block absolute top-10 left-[calc(50%+32px)] w-[calc(100%-64px)] h-0.5 bg-cyan-200 z-0" />
                )}

                <div className="relative z-10 flex items-center justify-center w-16 h-16 rounded-full bg-cyan-500 text-white text-2xl font-bold shadow-lg shadow-cyan-500/30 mb-5">
                  {item.step}
                </div>

                <div className="flex items-center justify-center w-14 h-14 rounded-full bg-white border border-gray-100 shadow-sm mb-4 group-hover:scale-110 transition-transform duration-300">
                  <Icon
                    name={item.icon}
                    size={26}
